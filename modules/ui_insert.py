...
class InsertUI:
    ...
    def build_insert_ui(self):
        ...
        # ── Tiêu đề ───────────────────────────────────────────────
        tk.Label(self.insert_frame, text="Auto Chèn (DS của bạn)",
    ...
    def _add_insert_row(self, is_first=False, slot_num=None, mua_ban_init="Mua"):
        ...
        def _open_slot_popup(b=slot_btn, v=slot_var):
            ...
            pop.geometry(f"{bw}x{11 * bh}+{bx}+{by}")
    ...
    def _add_insert_row_auto(self):
        """
        Bấm + THÊM: tính slot nhỏ nhất chưa dùng, sau đó query game
        (OCR cột Trạng thái) để tự chọn Mua/Bán cho slot đó.
        Nếu không tìm được status thì mặc định "Mua".
        """
        if len(self.insert_rows_data) >= _MAX_ROWS:
            return

        # Tính slot nhỏ nhất chưa có
        used = {int(r["slot"].get()) for r in self.insert_rows_data}
        next_slot = next((i for i in range(1, _MAX_ROWS + 1) if i not in used), 1)

        # Query game: OCR cột Trạng thái để lấy danh sách "Đang mua"/"Đang bán"
        def _query_and_add():
            mua_ban_init = "Mua"
            try:
                ...
                if not hwnd:
                    raise Exception("no hwnd")
                rect = win32gui.GetWindowRect(hwnd)
                gx1, gy1, gx2, gy2 = rect
                gw, gh = gx2 - gx1, gy2 - gy1

                # Cột Trạng thái nằm khoảng 72-88% chiều rộng game
                col_x1 = gx1 + int(gw * 0.72)
                col_x2 = gx1 + int(gw * 0.88)
                # Vùng danh sách: từ 20% đến 85% chiều cao
                col_y1 = gy1 + int(gh * 0.20)
                col_y2 = gy1 + int(gh * 0.85)

                # Chụp cột Trạng thái
                img = ImageGrab.grab(bbox=(col_x1, col_y1, col_x2, col_y2))
                iw, ih = img.size
                # Phóng to 3x để OCR chuẩn hơn
                img_big = img.resize((iw * 3, ih * 3))
                arr = np.array(img_big.convert("L"))
                # Threshold: chữ sáng trên nền tối
                _, bw = cv2.threshold(arr, 100, 255, cv2.THRESH_BINARY)
                from PIL import Image as PILImage
                pil = PILImage.fromarray(bw)

                # OCR toàn cột, lấy từng dòng
                ...
                # Gom các từ thành dòng theo block/line number
                statuses = []   # list of "Mua" | "Bán" theo thứ tự dòng từ trên xuống
                line_texts = {}
                for i, text in enumerate(data["text"]):
                    if not text.strip():
                        continue
                    line_key = (data["block_num"][i], data["line_num"][i])
                    line_texts.setdefault(line_key, []).append(
                        (data["top"][i], text.strip().lower())
                    )

                # Sort theo top y rồi phân loại
                sorted_lines = sorted(line_texts.items(), key=lambda x: x[1][0][0])
                ...
                # Lấy status của next_slot
                if len(statuses) >= next_slot:
                    mua_ban_init = statuses[next_slot - 1]

            except Exception:
                pass

            self.root.after(0, lambda: self._add_insert_row(
                is_first=False,
                slot_num=next_slot,
                mua_ban_init=mua_ban_init
            ))

        import threading
        threading.Thread(target=_query_and_add, daemon=True).start()
